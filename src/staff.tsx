...
function Tile({ service, compact = false }: { service: Service; compact?: boolean }) {
  ...
  return service.enabled ? <a className={className} href={service.url} aria-label={`${service.name} (opens in a new tab)`} target="_blank" rel="noopener noreferrer">{body}</a> : <article className={className} aria-disabled="true">{body}</article>;
}
