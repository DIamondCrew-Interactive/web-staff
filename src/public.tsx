...
import type { PublicSnapshot, ServerState, ServerStatus } from './shared/types';
import './styles.css';
import './launcher.css';
import './status.css';
...
const states: { state: ServerState; label: string; description: string }[] = [
  ...
  { state: 'DEGRADED', label: 'Degradovaný', description: 'Částečné omezení' },
...
function StatusCard({ server, updatedAt }: { server: ServerStatus; updatedAt: string }) {
  const [expanded, setExpanded] = useState(false);
  const visual = presentation[server.id], label = states.find(s => s.state === server.state)!.label;
...
function App() {
  ...
  const issues = data?.servers.filter(s => s.state === 'OFFLINE' || s.state === 'DEGRADED') || [];
  const maintenanceServices = data?.servers.filter(s => s.state === 'MAINTENANCE') || [];
  const maintenanceActive = !!data?.maintenance.active || maintenanceServices.length > 0;
  return <div className="launcher public-status-page"><a className="skip" href="#main">Skip to content</a><header className="launcher-header status-header"><a className="launcher-brand" href="https://staff.diamondcrew.net"><img src="/diamondcrew-logo.png" alt="DiamondCrew"/><span><strong>DC Interactive</strong><small>PLAY • CREATE • TOGETHER</small></span></a><nav className="status-navigation" aria-label="Hlavní navigace"><a href="https://staff.diamondcrew.net"><Home size={18}/><span>Home</span></a><a href="#services"><Monitor size={18}/><span>Services</span></a><a href="#main" aria-current="page"><Activity size={18}/><span>Status</span></a></nav><a className="button discord status-login" href="https://staff.diamondcrew.net/auth/discord"><LogIn size={15}/>Login with Discord</a><div className="header-shards" aria-hidden="true"/></header>
    ...
    <section className="status-summary" aria-label="Souhrn stavů">{states.map(item => <div className={`status-summary-item summary-${item.state.toLowerCase()}`} key={item.state}><span className="summary-light"/><div><strong>{data ? data.servers.filter(s => s.state === item.state).length : '—'}</strong><h2>{item.label}</h2><p>{item.description}</p></div></div>)}</section>
    <section id="services" className="public-service-grid" aria-label="Stav služeb">{data?.servers.map(server => <StatusCard key={server.id} server={server} updatedAt={data.updatedAt}/>)}{!data && <div className="status-loading">{loading ? 'Zjišťuji dostupnost služeb…' : 'Údaje o službách nyní nejsou dostupné.'}</div>}</section>
    <div className="status-bottom-grid"><section className="status-info-panel"><h2><Clock3 size={19}/>Aktuální hlášení</h2>{data?.incident && <div className="status-incident" role="alert"><CircleAlert size={20}/><div><h3>{data.incident.title}</h3><p>{data.incident.message}</p></div></div>}{issues.map(s => <div key={s.id} className="current-issue"><span className={`status-dot dot-${s.state.toLowerCase()}`}/><strong>{s.name}</strong><span>{states.find(item => item.state === s.state)!.label}</span></div>)}{!data?.incident && !issues.length && <div className="status-panel-empty"><Activity size={36}/><h3>{data ? 'Žádný nahlášený incident' : 'Hlášení se načítají'}</h3><p>{data?.servers.some(s => s.state === 'UNKNOWN') ? 'U služeb bez zdroje dat zatím nelze dostupnost potvrdit.' : 'Zde se zobrazí aktuální oznámení a zjištěné výpadky.'}</p></div>}</section>
