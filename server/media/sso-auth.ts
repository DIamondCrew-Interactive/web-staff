...
export function verifyImageAssertion(assertion: unknown, state: string, config: ImageSsoConfig, now = Date.now) {
  ...
  if (!config.allowedIds.has(claims.sub)) throw Error('No Image Service access');
  return claims as {sub:string; jti:string; exp:number};
}
export function createImageAuth(production: boolean, config: ImageSsoConfig | null, request: Fetcher = fetch, now = Date.now) {
  ...
  const prune = () => {for(const [k,v] of transactions) if(v.expires<=now()) transactions.delete(k); for(const [k,v] of sessions) if(v.expires<=now() || !config?.allowedIds.has(v.id)) sessions.delete(k); for(const [k,v] of used) if(v<=now()) used.delete(k);};
  const session = (req:Request) => {const k=cookie(req,sessionName); if(!opaque.test(k)) return; const s=sessions.get(k); if(s && (s.expires<=now() || !config?.allowedIds.has(s.id))) {sessions.delete(k);return;} return s;};
  ...
  function mount(app:Express) {
    ...
    app.get('/auth/sso/callback',async(req,res)=>{
      ...
      try {
        ...
        const claims=verifyImageAssertion(result.assertion,pending.state,config,now);prune();
        if(used.has(claims.jti) || sessions.size>=10000 || used.size>=10000) throw Error('Assertion already consumed or capacity reached');
        used.set(claims.jti,claims.exp*1000);sessions.delete(cookie(req,sessionName));const key=nonce();sessions.set(key,{id:claims.sub,csrf:nonce(),expires:now()+3600000});
